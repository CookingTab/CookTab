...
const DetailRecipe = () => {
  ...
  const handleBookmark = async () => {
    ...
    Swal.fire({
      title: "Success Added to Bookmark",
      icon: "success",
    });
  };
  ...
  return (
    <div className={bgcolor}>
      <div
        className={`${bgcomps} max-w-7xl mx-auto p-6 flex sm:flex-row flex-col-reverse gap-5 items-start`}
      >
        <div className={`${bgdetail} flex-1 rounded-sm bg-white space-y-3`}>
          <img
            src={recipe?.image}
        ...
        <div
          className={`${bgdetail} sm:w-80 w-full p-3 rounded-sm bg-white border shadow space-y-3 `}
        >
          {user ? (
